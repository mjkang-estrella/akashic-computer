  ...
  return (
    <article className="pb-12 pt-6">
      ...
      <header className="mt-4 border-b border-line pb-7">
        <p className="font-mono text-[11px] text-muted">{path.topic} · {path.level} · Learning path</p>
        <h1 className="mt-3 max-w-[25ch] font-display text-[36px] font-semibold leading-tight sm:text-[46px]">{path.title}</h1>
        <p className="mt-4 max-w-[70ch] text-[15px] leading-relaxed text-muted">{path.summary}</p>
        <p className="mt-4 max-w-[70ch] text-[14px] leading-relaxed"><strong>You’ll be able to: </strong>{path.outcome}</p>
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-[230px_minmax(0,1fr)]">
        <aside className="self-start rounded-[8px] border border-line bg-panel p-5 lg:sticky lg:top-5" aria-label="Path preparation">
          <h2 className="font-display text-[20px] font-semibold">Before you start</h2>
          {path.prerequisites.length ? <div className="mt-3 space-y-2">{path.prerequisites.map((slug) => <Link key={slug} href={`/docs/paths/${slug}`} className="block min-h-11 text-[12px] leading-relaxed underline underline-offset-4">{studyPath(slug)?.title} →</Link>)}</div> : <p className="mt-3 text-[12px] leading-relaxed text-muted">No earlier path required. Start with the first resource and use the guides when you meet an unfamiliar concept.</p>}
          <h3 className="mt-5 border-t border-line pt-4 text-[12px] font-semibold">Akashic companion guides</h3>
        ...
        <div className="min-w-0">
          <section aria-labelledby="reading-order-title">
            ...
            <p className="mt-2 text-[12px] text-muted">Follow this order. Mark a resource as read when you’ve worked through it.</p>
          ...
          <section id="investigation" aria-labelledby="investigation-title" className="mt-8 scroll-mt-5 rounded-[8px] border border-ink bg-paper p-5 sm:p-6">
            <p className="font-mono text-[10px] uppercase tracking-[0.1em] text-muted">Put it to work · Investigation</p>
            <h2 id="investigation-title" className="mt-2 font-display text-[26px] font-semibold">{path.exercise.title}</h2>
