...
import { artifactsFor, bestRunnable, fitOf } from "@/lib/atlas/fit";
import { activeParamsLabel, sizeDisplay, uploaderDisplay } from "@/lib/atlas/naming";
import type { Family, Release, RigProfile, SizeNode } from "@/lib/atlas/types";
import { ConfidenceNote, FitBadge, PropertyChip, TrustBadge } from "./badges";
...
function SizeFitDot({
  ...
  const v = size.variants.includes(variant) ? variant : size.variants[0];
  const best = bestRunnable(artifactsFor(family, release, size, v), rig);
  const color = best
    ? best.fit.level === "runs"
      ? "bg-verify"
      : "bg-caution"
    : "bg-alert";
  return <span className={`h-[7px] w-[7px] flex-none rounded-full ${color}`} />;
}
  ...
  return (
    <div className="grid gap-4 pt-4 md:grid-cols-[248px_minmax(0,1fr)]">
      ...
      <section>
        ...
        <div className="mt-3.5 grid items-start gap-3.5 xl:grid-cols-[minmax(0,1fr)_300px]">
          <div className="rounded-[10px] border border-line bg-panel px-4 py-3.5">
            <div className="mb-1 flex flex-wrap items-center justify-between gap-2.5">
              ...
              <span className="text-xs text-faint">
                Check two or more to compare quantizations
              </span>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[540px] border-collapse">
                <thead>
                  <tr>
                    {["", "Artifact", "Runtimes", "VRAM", "Fit"].map(
                      (h, i) => (
                        <th
                          key={i}
          ...
          <aside className="rounded-[10px] border border-line bg-panel px-4 py-3.5">
            <h2 className={`mb-2.5 ${SECTION_LABEL}`}>Benchmarks</h2>
            {scores ? (
              BENCHES.map((b) => (
                <div
                  key={b.key}
                  className="grid grid-cols-[1fr_auto_auto] items-baseline gap-2 border-b border-linesoft py-1.5 last:border-b-0"
                >
            ...
            <div className="mt-3.5 rounded-lg bg-panel2 px-3 py-2.5 text-[12.5px] text-muted">
              <b className="text-ink">Why so many repos for one model?</b>
              <br />
              The vendor ships the reference checkpoint. NVIDIA, AWQ, GGUF and
              MLX repos re-package it for different hardware — smaller, faster,
              slightly less accurate. Trust and quality deltas are tracked per
              repo.
            </div>
          </aside>
        </div>
